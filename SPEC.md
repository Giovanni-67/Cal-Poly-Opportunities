...
- **Experience:** Follow the approved September 14 editorial redesign: warm off-white page, forest green controls, restrained gold focus accents, serif headings with sans-serif interface text, compact filters, a dated career agenda, and opportunity listings separated by thin rules. Group study locations by building and campus resources by need; use a compact bookmarks list. Clicking a listing animates its details into the center as the background blurs; expandable details reveal a Learn more link. Keep keyboard controls, focus management, Escape dismissal, and reduced-motion support.
...
## Approved editorial redesign — September 14, 2026

The student approved the [desktop/mobile proposal](design/README.md) and authorized implementation on a new branch with meaningful commits, pushes, PRs, and merges. Keep all ten interest choices, existing matching and ordering, related activities, sources, bookmarks, event expiry, and bug reporting. Event/company filters can use an expandable search area while the agenda remains visible. Use real photography only with verified reuse rights and clear context. This refresh changes presentation and campus grouping; it does not establish new opportunity facts or change publishing settings.
